...
$error_log = "../../endpoint_logs/story_create_error.log";

$success_log = "../../endpoint_logs/story_create_success.log";
...
if(empty($run_auth_check)){
    ...
    if (!empty($request_body)) {
        ...
        if ($decoded_body !== null) {
            ...
            $body_whitelist = ['title','date_of_event','text'];
            ...
            $query = $wpdb->prepare(
                "INSERT INTO xx_timeline_items
                (date_of_event, event_title, event_text)
                VALUES (%s, %s, %s)",
                $decoded_body['date_of_event'],
                $decoded_body['title'],
                $decoded_body['text'],
            );

            // Insert the new timeline event
            $insert_new_story = $wpdb->query($query);

            if($insert_new_story != 1){
                ...
                $logMessage = "Timeline Event successfully inserted.";
